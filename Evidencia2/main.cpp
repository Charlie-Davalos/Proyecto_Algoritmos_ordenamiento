...
template <class T> void outputData(DList<DList<T>> data, int start, int end);
template <class T>
DList<DList<T>> sortData(DList<DList<T>> &data, int start, int end,
int varIndex);
void casosPrueba();
void search(string value, int varName,
            DList<BST<string, DList<string>>> &bstList);
...
int main() {
  ...
  // Define the list of BST's
    ...
    do {
    cout << "\nWelcome to the the CO2 emissions Worldwide database, please "
            "select an option\n";
    cout << "0. Exit\n";
...
DList<DList<string>> readData(DList<BST<string, DList<string>>> &bst_list) {
  ...
  // Read file
    ...
    // IMPORTANTE: Como estoy en Repl.it, no tengo mucha memoria disponible, por
